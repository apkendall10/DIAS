...
def make_cpu_graph():
	...
	df=pd.DataFrame({'# of clients': num_clients, 'Auto-Scale': cpu_usage_auto_scale, 'Static': cpu_usage})
...
def make_throughput_graph():
		...
		for client in sorted(clients):
			...
			for file in os.listdir(path):
				if "total" in file:
					...
					th.append(data_size/total_latency)
	...
	df=pd.DataFrame({'# of clients': num_clients , 'Auto-Scale': throughput_auto_scale, 'Static': throughput})
